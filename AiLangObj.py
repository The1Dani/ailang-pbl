...
class AiLangObj:
    ...
    def set(self, val:AiLangType.AiLangType) -> None:
        self.val = val
    ...
    def getDFItemByID(self, id):
        items = filter(lambda v: isinstance(v.val, AiLangType.DfItem),list(self.members.values()))
        for item in items:
            if item.id == id:
                return item
    ...
    def make(node:ap.AssignableContext) -> AiLangObj:
        ...
        if isinstance(node, ap.MemberTargetContext):
            ...
            member_obj = evalMember(node.getTypedRuleContext(ap.MemberContext, 0), assignable_obj)
            assignable_obj.setMember(member_obj)
            return assignable_obj
        raise ValueError()
    
    def __repr__(self) -> str:
        return f"{self.id} -> {repr(self.members)}"
